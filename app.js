...
class Hades {
  ...
  async main() {
    ...
    if (BT_BRL.data.result[0].Balance > 0) {
      // await Bitrecife.setSellLimit('USDT_BRL',  BT_BOOK_USDT_BRL.data.result.sell[0].Rate, BT_BRL.data.result[0].Balance);
    ...
    this.qnt_BRL = (this.qnt_BTC * (BT_BTC_BRL.data.result[0].Bid / BT_USDT_BRL.data.result[0].Bid)) * (1 - this.rateBt);

    // Profit to arbitration
    const profit = this.percent(this.qnt_BRL, this.usdt);
